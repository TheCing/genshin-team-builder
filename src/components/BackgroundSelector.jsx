...
export default function BackgroundSelector({
  ...
  // Initialize with the stored background or default to none
  ...
  useEffect(() => {
    if (selected) {
      if (selected !== "none") {
        document.body.style.backgroundImage = `linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url(${selected})`;
      } else {
  ...
  }, [selected, onSelectBackground]);

  // If selectedBackground prop changes, update the selected state
  useEffect(() => {
    if (selectedBackground && selectedBackground !== selected) {
        ...
        {backgrounds.map((bgPath) => (
          <div
            ...
            className={`background-selector__item ${
              selected === bgPath ? "background-selector__item--selected" : ""
            }`}
            onClick={() => setSelected(bgPath)}
          >
            {bgPath === "none" ? (
